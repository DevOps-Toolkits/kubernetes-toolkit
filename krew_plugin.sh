...
(
    wget https://github.com/stern/stern/releases/download/v1.25.0/stern_1.25.0_linux_amd64.tar.gz -O - | tar -xz
    chmod +x stern && \
...
(
    set -x &&
    wget -c https://github.com/chenjiandongx/kubectl-images/releases/download/v0.5.2/kubectl-images_linux_amd64.tar.gz -O - | tar -xz &&
    chmod +x kubectl-images &&
    mv kubectl-images /usr/local/bin/
)

# need to updated manually
